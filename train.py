...
if __name__ == '__main__':
    ...
    dataset = ModelNet(split="train", npoints=opt.num_points, classes=["bed"])
    dataloader = torch.utils.data.DataLoader(dataset, batch_size=opt.batchSize,
                                              shuffle=True, num_workers=int(opt.workers))
    # dataset_test = ShapeNet(train=False, npoints=opt.num_points)
    dataset_val = ModelNet(split="val", npoints=opt.num_points, classes=["bed"])
        ...
        if (epoch + 1) % 5 == 0:
            ...
            with torch.no_grad():
                for i, data in enumerate(dataloader_test, 0):
                    ...
                    print(opt.env + ' val [%d: %d/%d]  emd1: %f emd2: %f expansion_penalty: %f' % (epoch, i, len(dataset_val) / opt.batchSize, emd1.mean().item(), emd2.mean().item(), expansion_penalty.mean().item()))
